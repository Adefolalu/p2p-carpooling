...
export default function Search() {
  ...
  const searchRides = async () => {
    ...
    const contract = new ethers.Contract(
      "0xe5b0c9C57f4438462492f267169a8D6412Cf21C0",
      abi,
      provider
    );
    ...
    for (let i = 0; i < rideCount; i++) {
      ...
      if (
        ...
        Number(ride.seats._hex.toString()) > 0
      ) {
        validRides.push(ride);
      }
  ...
  const bookRide = async (rideId) => {
    ...
    const rideOwnerAddress = await contract.rideowner(rideId);
    const ride = await contract.rides(rideId);
    const rideOwnerPhoneNumber = ride.phoneNumber;

    const _bookRide = await contract.bookRide(rideId);
    await _bookRide.wait();
    console.log(_bookRide);
    toast(
      `Ride booked successfully. Ride owner's phone number: ${rideOwnerPhoneNumber}`
    );
  };
                  ...
                  {rides.map((ride, index) => {
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                          <button
                            className="bg-blue text-gray-900 px-4 py-1 rounded-lg text-center "
                            onClick={() => {
